...
'''
This is a Conway's Game of Life simulation, using dictionaries.
...
class LifeGrid():
    ...
    def runLife(self):
        ...
        for y in range(self.size[1]):
            for x in range(self.size[0]):
                neighbors = (laststate[x, (y-1)%self.size[1]] + laststate[x, (y+1)%self.size[1]] +
                    laststate[(x-1)%self.size[0], y] + laststate[(x+1)%self.size[0], y] +
                    laststate[(x-1)%self.size[0], (y-1)%self.size[1]] + laststate[(x-1)%self.size[0], (y+1)%self.size[1]] +
                    laststate[(x+1)%self.size[0], (y-1)%self.size[1]] + laststate[(x+1)%self.size[0], (y+1)%self.size[1]])
                # rules
    ...
    def poke(self, pos, state):
        spot = ((pos[0]-2)//PRATIO), ((pos[1]-2)//PRATIO)  # -2 kinda fixes rounding
